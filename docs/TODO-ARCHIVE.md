# TODO archive — the session logs, moved out of `TODO.md` on 2026-09-07

`TODO.md` describes itself as *only what is left to do* and had grown to 1,931 lines of dated
session logs, most of them done-lists with the open items buried inside. The open items were
consolidated into the new `TODO.md`; everything below is kept **verbatim** as history, because the
measurements and reasoning in it are cross-referenced from `docs/DECISIONS.md` by date and item
number. **Nothing here is current.** Where a line below names a file that no longer exists
(`docs/DNSSEC-TICKET.md`, `docs/REVIEW-2026-08-16.md`, `docs/AUDIT-BRIEF.md` — all retired the
same day as superseded), the history stands as written.
