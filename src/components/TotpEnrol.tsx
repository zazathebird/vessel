  ...
  if (enrolment) {
    return (
      <form className="v-account-form" onSubmit={onConfirm}>
        ...
        <p className="v-account-note">
          Scan this with your authenticator app. If it will not scan, type the secret in by hand
          instead, or paste the address into an app that accepts one — all three carry the same
          key.
        </p>
        ...
        {/*
          * The secret and the URI stay, and not as a courtesy. The QR is
          * generated by hand-written code (`src/auth/qr.ts`) with no library
          * behind it, so if it is ever wrong it will be wrong silently — a
          * symbol that simply does not scan. Leaving both text forms means the
          * failure costs a minute of typing rather than locking somebody out of
          * the factor they are in the middle of setting up.
          */}
