...
// Types only: the engine itself is a lazy chunk — see `loadDuelEngine`.
...
export type Effect = (f: Frame, cache: FxCache) => void;
...
/*
 * The engine, once it has arrived. Null until `loadDuelEngine` resolves.
 *
 * **The duels are a lazy chunk, and this is the only door into it**
 * (2026-09-24). See `duelEffect.ts` for why. Nothing in the entry bundle may
 * import `./duel`, `./fighters` or `./duelEffect` at runtime — types are fine,
 * they are erased — and `npm run check` reads the built entry to prove it.
 */
let duelEffects: Record<DuelPool, Effect> | null = null;
let duelLoading: Promise<void> | null = null;

/**
 * Fetch the duel engine, once. Called by the duel effects on their first frame,
 * which is to say only when a duel is what is actually being drawn — and that
 * only ever happens for the operator, because `ConfigContext` substitutes the
 * duels out for everybody else. A failed fetch is forgotten, so the next frame
 * tries again rather than leaving the operator with a blank ground for the life
 * of the tab. Exported so the gate suite can await it before driving the fight.
 */
export function loadDuelEngine(): Promise<void> {
  if (duelEffects) return Promise.resolve();
  duelLoading ??= import("./duelEffect").then(
    (m) => {
      duelEffects = m.DUEL_EFFECTS;
    },
    (error: unknown) => {
      duelLoading = null;
      throw error;
...
function lazyDuel(pool: DuelPool): Effect {
  return (f, cache) => {
    if (duelEffects) {
...
const duel = lazyDuel("duel");
const duelholy = lazyDuel("duelholy");
